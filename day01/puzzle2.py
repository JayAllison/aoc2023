...
digit_regex_forward: re.Pattern = re.compile(r'\d|one|two|three|four|five|six|seven|eight|nine')
digit_regex_reverse: re.Pattern = re.compile(r'\d|eno|owt|eerht|ruof|evif|xis|neves|thgie|enin')
...
for line in open(input_filename):
    ...
    first_digit: str = digit_regex_forward.search(line)[0]
    # use regex search the reversed string for the last digit, either the number or the name
    last_digit: str = digit_regex_reverse.search(line[::-1])[0]
    number_string: str = conversion[first_digit] + conversion[last_digit]
