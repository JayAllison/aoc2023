    ...
    for pipe_map_y in range(len(pipe_map)):
        for pipe_map_x in range(len(pipe_map[pipe_map_y])):
            if pipe_map[pipe_map_y][pipe_map_x] == 'S':
                return pipe_map_y, pipe_map_x
    return None, None


def step_away_from_s(pipe_map, s_y, s_x) -> tuple[int | None, int | None, str | None]:
    # check each direction and find one that leaves S
...
def get_right_outside(direction: str):
    match direction:
        case 'north':
            return 'east'
        case 'south':
...
def mark_outside(pipe_map: list, y: int, x: int, direction, path_points) -> None:
...
def main():
    ...
    while current_y != s_y or current_x != s_x:
        mark_outside(pipe_map, current_y, current_x, get_right_outside(current_direction), path_points)
        next_y, next_x, next_direction = get_next_step(pipe_map, current_y, current_x, current_direction)
        # mark again after turning corner - will still be missing the diagonal outside, but it does not affect outcome
        if next_direction != current_direction:
            mark_outside(pipe_map, current_y, current_x, get_right_outside(next_direction), path_points)
