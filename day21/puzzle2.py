...
input_filename: str = 'sample_data'; steps_needed = 1000
...
class GardenStepper:
    ...
    def get_valid_next_steps(self, loc: tuple) -> list[tuple]:
        steps = []
        y, x = loc
        potentials = [(_y, x) for _y in (y - 1, y + 1)]
        potentials += [(y, _x) for _x in (x - 1, x + 1)]
        for py, px in potentials:
            # Part 2: the map *repeats infinitely* in every direction, so map this point back to our little garden map
            _py = py % self.max_y
            _px = px % self.max_x
            if self.garden_map[_py][_px] in '.S':  # potential is a valid next step if it is garden ('.' or 'S')
                steps.append((py, px))
        return steps
...
if __name__ == '__main__':
    ...
    print(f'Steps: {steps_needed} -> {GardenStepper(input_filename, steps_needed).solve()} garden plots, time = {datetime.datetime.now() - start}')
